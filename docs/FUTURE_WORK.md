...
`flutter_launcher_icons: ^0.14.1` (dev_dependency) 로 단일 PNG 마스터에서 iOS/Android 모든 사이즈 자동 생성.

- 마스터: `assets/icon/app_icon.png` (1024×1024, 투명 배경 + shield 중앙)
- pubspec 설정: `remove_alpha_ios: true` + `background_color_ios: "#0D1117"` (iOS 투명 영역을 다크 bgPrimary 로 fill, opaque 보장 — Apple 심사 요건)
- Android adaptive: `adaptive_icon_background: "#0D1117"` + `adaptive_icon_foreground` 동일 PNG (Android 8+ 기기에서 background drawable 위에 shield 만 떠있는 형태)
- AndroidManifest.xml: `android:icon="@mipmap/launcher_icon"` 자동 갱신
- 생성 위치:
  - iOS: `ios/Runner/Assets.xcassets/AppIcon.appiconset/*.png` (20×20 ~ 1024×1024)
  - Android: `android/app/src/main/res/mipmap-*/launcher_icon.png`, `drawable-*/ic_launcher_foreground.png`, `mipmap-anydpi-v26/launcher_icon.xml`, `values/colors.xml`

App Store / Play Store 출시 시 별도 1024×1024 스토어 아이콘은 `assets/icon/app_icon.png` 그대로 또는 약간의 변형(스토어 룰에 맞춘 padding 등) 으로 사용 가능.
